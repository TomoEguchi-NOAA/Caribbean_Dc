...
In this version (v.6), another parameter (probability of nesting) was added to accommodate the fact that not all females nest every year. No latitude covariate is included.

In this version (v.7), I greatly simplified the model to have a total abundance (N), which is subdivided into 73 beaches. Then for each beach the same proportion (p) nests annually. Then, they are observed - should add a covariate for observation effort. 
...
year.mat <- y <- matrix(nrow = nrow(summary.years), 
                        ncol = max(summary.years$year.2) - min(summary.years$year.1) + 1)

min.year <- min(summary.years$year.1)
...
for (k in 1:nrow(summary.years)){
  ...
  y[k,tmp$col.yr] <- tmp$nests
  year.mat[k, tmp$col.yr] <- tmp$year
  
  if (sum(is.na(year.mat[k, ])) > 0)
    NA.idx[[k]] <- c(1:ncol(year.mat))[is.na(year.mat[k, ])]
}
```
...
```{r}
year.vec <- seq(from = min(summary.years$year.1), 
                to = max(summary.years$year.2))
years <- ifelse(year.vec < 2011, 1, 2)
# k <- 3
# for (k in 1:nrow(year.mat)){
...
MCMC.params$model.file = "models/Model_norm_Pois_2Us_skip_Nsum.txt"

if (!file.exists("RData/JAGS_out_norm_Pois_2Us_skip_Nsum.rds")){
  ...
  saveRDS(jm, file = "RData/JAGS_out_norm_Pois_2Us_skip_Nsum.rds")
  
} else {
  jm <- readRDS("RData/JAGS_out_norm_Pois_2Us_skip_Nsum.rds")
}
...
These parameters are not okay; Rhat stats are > 1.1.   
...
Dirichlet parameters didn't converge well.
...
for (p in 1:length(NA.idx)){
  ...
  #Nt <- exp(log.Nt) - 1
  Nt$year <- seq(from = min(summary.years$year.1), 
                to = max(summary.years$year.2))
  ...
  ggsave(p.1.2, 
         filename = paste0("figures/double_Us/beach_", 
                           p, "_norm_Pois_2Us_skip_Nsum.png"),
         device = "png", dpi = 600,
...
```{r}
mcmc_dens(jm$samples, pars = c("r[1]", "r[2]", "r[3]", "r[4]"))
```



```{r}
mcmc_dens(jm$samples, pars = c("N0[1]", "N0[2]", "N0[3]", "N0[4]"))
```

```{r}
mcmc_dens(jm$samples, pars = c("N0[21]", "N0[22]", "N0[23]", "N0[24]"))
```


```{r}
mcmc_dens(jm$samples, pars = c("p"))
```
