...
high.low <- matrix(nrow = nrow(years), ncol = ncol(years))

#k <- 1 k1 <- 2
# Even years are 1s and odd years are 2s
for (k in 1:nrow(high.low)){
  high.low[k, 1] <- ifelse(year.mat[k, 1] %% 2 == 0, 1, 2)
  for (k1 in 2:ncol(high.low)){
    high.low[k, k1] <- ifelse(high.low[k, (k1-1)] %% 2 == 0, 1, 2)
  }
}
...
MCMC.params$model.file = "models/Model_norm_Pois_Qs_2Us_skip.txt"

if (!file.exists("RData/JAGS_out_norm_Pois_Qs_2Us_skip.rds")){
  ...
  saveRDS(jm, file = "RData/JAGS_out_norm_Pois_Qs_2Us_skip.rds")
  
} else {
  jm <- readRDS("RData/JAGS_out_norm_Pois_Qs_2Us_skip.rds")
}
...
These parameters look okay; Rhat stats are < 1.1.   
